...
class PipelineSpec extends AnyFlatSpec with ChiselScalatestTester {
  ...
  def pipelineTest(instructions: Seq[String], memory: Seq[(Int,Int)])(pcChange: (Long, Long))(regChange: Map[Int,(Long,Long)]): TestResult = {
    ...
    test(new Pipeline(initialState)).withAnnotations(Seq(WriteVcdAnnotation)) { dut =>
      ...
      val mem = new Memory(dut)(assembleToBytes(instructions).zipWithIndex.map(t => (t._2,t._1)) ++ memory)
  ...
  class Memory(dut: Pipeline)(init: Seq[(Int,Int)]) {
    ...
    def step(): Unit = {
      ...
      if(instrRequestPipe) {
        ...
        val instr = mem(instrReadPipe).toLong | (mem(instrReadPipe + 1) << 8).toLong  | (mem(instrReadPipe + 2) << 16).toLong  | (mem(instrReadPipe + 3) << 24).toLong
      ...
      dataRequestPipe = dut.io.dataChannel.request.valid.peek.litToBoolean && (dut.io.dataChannel.request.bits.op.peek == MemoryOperation.Read)
